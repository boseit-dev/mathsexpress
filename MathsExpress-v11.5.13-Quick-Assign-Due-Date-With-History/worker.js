...
async function __mxProxySignup(request){
  ...
  try{
    ...
    const upstream=await fetch(`${MX_PUBLIC_SUPABASE_URL}/functions/v1/mathsexpress-signup`,{
      method:'POST',
      headers:{'Content-Type':'application/json','apikey':MX_PUBLIC_SUPABASE_KEY,'Origin':new URL(request.url).origin},
...
export default {
  async fetch(request, env){
    ...
    assetHeaders.set('X-Frame-Options','DENY');
    assetHeaders.set('Referrer-Policy','no-referrer');
    assetHeaders.set('Permissions-Policy','camera=(), microphone=(), geolocation=(), payment=(), usb=(), browsing-topics=(), display-capture=()');
    assetHeaders.set('Cross-Origin-Opener-Policy','same-origin');
    assetHeaders.set('X-Permitted-Cross-Domain-Policies','none');
    if(routePath==='/app.html') assetHeaders.set('X-Robots-Tag','noindex, nofollow');
