...
class MathRiftAccountClient {
  ...
  async signIn(login, password) {
    ...
    // Always use the server login route. It also upgrades old unverified accounts
  ...
  async resendSignupVerification(email) { return true; }
  ...
  async signUp(email, password, displayName, accountType = 'student') {
    ...
    try{
      ...
      try{
        response=await fetch('/api/signup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      }catch{
        throw new Error('Could not reach the account service. Check your internet connection and try again.');
      }
    }
    ...
    if(!result?.access_token || !result?.refresh_token) throw new Error('Account was created but sign-in could not start. Please log in.');
    const {data,error}=await client.auth.setSession({access_token:String(result.access_token),refresh_token:String(result.refresh_token)});
    if(error) throw error;
    this.session=data.session||null;
    await this.refreshProfile();
...
class MathRiftSchoolClient {
  ...
  async ownerDeleteUser(userId, confirmEmail) {
    const { data, error } = await this.client.functions.invoke('mathsexpress-owner-delete-user', {
      body: { userId:String(userId), confirmEmail:String(confirmEmail || '').trim().toLowerCase() },
    });
    if (error) throw error;
  ...
  async createSafetyAlert({ classId = null, category = 'other', source = 'site', snippet = '', severity = 'medium', details = {} } = {}) {
    return unwrap(await this.client.rpc('mathsexpress_create_safety_alert', {
      p_class_id:classId ? String(classId) : null,
      p_category:String(category || 'other'),
      p_source:String(source || 'site').slice(0,80),
      p_snippet:String(snippet || '').slice(0,300),
      p_severity:String(severity || 'medium'),
      p_details:details || {},
    }));
  }

  async teacherSafetyAlerts(schoolId, limit = 50) {
    return unwrap(await this.client.rpc('mathsexpress_teacher_safety_alerts', {
      p_school_id:String(schoolId), p_limit:Math.max(1,Math.min(200,Number(limit)||50)),
    })) || [];
  }

  async reviewSafetyAlert(alertId) {
    return unwrap(await this.client.rpc('mathsexpress_review_safety_alert', { p_alert_id:Number(alertId) }));
  }
